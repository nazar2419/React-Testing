...
export const calculatePopularLanguages = (
  ...
  return Object.entries(languageMap) // Convert entries into array of [language, count] pairs
    .sort(([, a], [, b]) => b - a) // Sort by count in descending order
    .slice(0, 5) // Take only the top 5 languages
    .map(([language, count]) => ({ language, count })); // Transform into required object format
};
